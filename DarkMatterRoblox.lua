--[[
    ...
    Update: ESP Highlight Separado (Bordes / Relleno) + Rapid Fire + Mobile Aim Fix + No Recoil.
...
local State = {
    ...
    AimbotMobile = false,
    NoRecoil = false, -- NUEVA OPCIÓN: No Recoil
...
CreateSlider("CAMPO DE VISIÓN (FOV)", 10, 120, State.FieldOfView, function(v) 
    State.FieldOfView = v
...
CreateSlider("Tamaño del FOV", 10, 400, State.FOVSize, function(val) State.FOVSize = val; FOVFrame.Size = UDim2.new(0, val*2, 0, val*2) end)
...
-- // IMPLEMENTACIÓN DEL HOOK (MOBILE AIM FIX) //
local _idx
